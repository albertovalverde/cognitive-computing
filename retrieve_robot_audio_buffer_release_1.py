...
class SoundReceiverModule(naoqi.ALModule):
    ...
    def start( self, nSampleRate = 48000, nChannelConfiguration = 0, bSaveOneFilePerChannel = False, bExportAsWav = True, bVerbose = False ):
        """
        ...
        ad.setClientPreferences( self.getName(),  self.nSampleRate, nChannelConfiguration, nDeinterleave ); # setting same as default generate a bug !?!
    ...
    def processRemote( self, nbOfChannels, nbrOfSamplesByChannel, aTimeStamp, buffer ):
        ...
        """
        if self.bVerbose: print( "DBG: processRemote received: nbOfChannels: %d, nbrOfSamplesByChannel: %d, aTimeStamp: %s, len(buffer):%d" % (nbOfChannels, nbrOfSamplesByChannel, str(aTimeStamp), len(buffer)) )
        ...
        if 1:
            yourOwnProcess( self.nSampleRate, nbOfChannels, nbrOfSamplesByChannel, aTimeStamp, buffer ) # add here your own processing doing whatever you want...
            ...
            if self.aOutFileDesc[0] == None:
                ...
                if self.bExportAsWav:
                    ...
                    if not self.bSaveOneFilePerChannel:
                        ...
                        wavTempJustForHeader.new( nSamplingRate = self.nSampleRate, nNbrChannel = nbOfChannels, nNbrBitsPerSample = 16 );
                        wavTempJustForHeader.writeHeader( self.aOutFileDesc[0], bAddBeginOfDataChunk = True );
                    else:
                        for nNumChannel in range( 0, nbOfChannels ):
                            wavTempJustForHeader = abcdk.sound.Wav();
                            wavTempJustForHeader.new( nSamplingRate = self.nSampleRate, nNbrChannel = 1, nNbrBitsPerSample = 16 );
...
def yourOwnProcess2():
    ...
    implement here whatever you want
    """
    SoundReceiverModule.stop(SoundReceiverModule)
...
def main():
    ...
    SoundReceiver.start( nSampleRate = nSampleRate, nChannelConfiguration = nChannelConfiguration, bSaveOneFilePerChannel = bSaveOneFilePerChannel, bExportAsWav=bExportAsWav, bVerbose = bVerbose )
    print "hice start"
    try:
        while True:
            time.sleep(1)
